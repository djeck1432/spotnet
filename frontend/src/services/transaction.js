...
const {account } = useAccountStore();
 ...
 async function sendTransaction(loopLiquidityData, contractAddress) {
  try {
    ...
    let result = await account.execute([approveTransaction, depositTransaction]);
 ...
 async function sendWithdrawAllTransaction(data, userContractAddress) {
  try {
    ...
    let result = await account.execute(
        [
 ...
 async function sendExtraDepositTransaction(deposit_data, userContractAddress) {
  try {
...
async function waitForTransaction(txHash) {
  ...
  while (receipt === null) {
    try {
      receipt = await account.provider.getTransactionReceipt(txHash);
    } catch (error) {
 ...
 async function closePosition(transactionData) {
  const callData = new CallData(abi);
 ...
 const handleTransaction = async (connectedWalletId, formData, setTokenAmount, setLoading) => {
  setLoading(true);
  try {
...
return {handleTransaction, sendExtraDepositTransaction, sendTransaction, sendWithdrawAllTransaction, closePosition}
