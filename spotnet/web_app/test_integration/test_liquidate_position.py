...
class TestPositionLiquidation:
    ...
    def test_position_liquidation(self, position_data: Dict[str, Any]) -> None:
        ...
        with with_temp_user(wallet_id):
            ...
            assert not position.is_liquidated, (
                "Position should not be liquidated initially"
            )
            assert position.liquidation_bonus == 0.0, (
                "Initial liquidation bonus should be 0"
            )
            assert position.datetime_liquidation is None, (
                "Liquidation datetime should be None initially"
            )

            # Open position
            current_prices = asyncio.run(DashboardMixin.get_current_prices())
            assert token_symbol in current_prices, (
            ...
            assert liquidated_position is not None, (
                "Should be able to retrieve liquidated position"
            )
            assert liquidated_position.is_liquidated is True, (
                "Position should be marked as liquidated"
            )
            assert liquidated_position.datetime_liquidation is not None, (
            ...
            assert len(all_liquidated) > 0, (
                "Should have at least one liquidated position"
            )
            assert any(
